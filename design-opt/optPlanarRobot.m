...
clear, clc
user = 'lucas';
addpath(['C:\Users\',user,'\Dropbox (GaTech)\Research\Hexapod\analysis\muscle force analysis'])
global t_comp0
t_comp0 = now;
...
animation = 1;
anim_delay = 0.00;
vid_export = 1;       % animation export on
vid_file = 'jump_vert_fromStand'; % animation video file name
...
sv_knee = combvec(opt_param.rad_knee, opt_param.slope_knee)';
sv_hip = combvec(opt_param.rad_hip, opt_param.slope_hip)';
sv = [sv_knee; sv_hip(~isnan(sv_hip(:,1)) & ~isnan(sv_hip(:,2)),:)];
sv = unique(sv,'rows'); % sweep vector of just knee/hip cam radius & slope
knee_joint.update_cam_data(sv);
hip_joint.update_cam_data(sv);
