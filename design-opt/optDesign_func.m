function [param_vec_opt, vy_jump_opt]  = optDesign(robot, opt_type,...
    sim_param, opt_param, export_opt_params)
    ...
    if export_opt_params == 1
        exportOptParams(nan, nan, vy_jump_opt, param_vec_opt)
    end
...
function stop = fminOutputFcn(x, optimvals, ~)
    ...
    if export_opt_params == 2            
        exportOptParams(optimvals.iteration, nan, vy_jump, param_vec)
    end     
...
function stop = fminPlotFcn(~,optimvals,~)
    ...
    persistent f
    if isempty(f)
        f = figure('Name', 'fmincon progress', 'Number', 1111); clf
        hold on
    end
    figure(f);
